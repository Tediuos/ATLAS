...
<div class="wrap">
  ...
  <div class="score-card">
    <div class="badge {{ 'good' if s >= 70 else ('medium' if s >= 50 else 'poor') }}">{{ s ~ '/100' if audit.score is not none else 'Unavailable' }}</div>
  ...
  <table>
    <thead><tr><th>Keyword</th><th>Cluster</th><th>Intent</th><th>Heuristic Rank</th></tr></thead>
    <tbody>
...
def generate_report(
    ...
    if publish:
        ...
        if not str(publish.get("link", "")).startswith(("http://", "https://")):
            publish["link"] = "#"
